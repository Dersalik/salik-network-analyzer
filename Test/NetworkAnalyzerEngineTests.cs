...
namespace Tests
{
    public class NetworkAnalyzerEngineTests
    {
        ...
        #region AnalyzeSingleDeviceAsync Tests

        [Fact]
        public async Task AnalyzeSingleDeviceAsync_WithLoopback_ShouldSucceed()
        {
            // Arrange
            ...
            result.IfRight(device =>
            {
                ...
                Assert.True(device.ResponseTime.IsSome);
                Assert.Equal(IPStatus.Success, device.PingStatus.Match(Some: s => s, None: () => IPStatus.Unknown));
                Assert.True(device.DeviceType != NetworkDeviceType.Unknown || device.HostName.IsNone);
            });
        }

        [Fact]
        public async Task AnalyzeSingleDeviceAsync_WithCustomPingConfig_ShouldUseConfig()
        {
            // Arrange
            var target = IPAddress.Loopback;
            var customConfig = new PingConfiguration(1000, 16, 32, false);
            ...
            // Act
            var result = await NetworkAnalyzerEngine.AnalyzeSingleDeviceAsync(target, customConfig);
        ...
        [Fact]
        public async Task AnalyzeSingleDeviceAsync_WithDefaultConfig_ShouldUseDefaults()
        {
            // Arrange
            var target = IPAddress.Loopback;

            // Act
            var result = await NetworkAnalyzerEngine.AnalyzeSingleDeviceAsync(target, default);
        ...
        [Fact]
        public async Task AnalyzeSingleDeviceAsync_WithUnreachableAddress_ShouldReturnUnreachableDevice()
        {
            // Arrange - Using a test address that should be unreachable
            var target = IPAddress.Parse("192.0.2.1"); // RFC 5737 test address
            var quickConfig = new PingConfiguration(100, 32, 64, true); // Short timeout

            // Act
            var result = await NetworkAnalyzerEngine.AnalyzeSingleDeviceAsync(target, quickConfig);
            ...
            result.Match(
                ...
                {
                    Assert.Equal(target, device.IpAddress);
                    // May be reachable or unreachable depending on network
                },
                Left: error =>
                {
                    Assert.IsType<NetworkError.NetworkDiscoveryFailed>(error);
        ...
        [InlineData("::1")]
        public async Task AnalyzeSingleDeviceAsync_WithDifferentAddressTypes_ShouldWork(string ipString)
        {
            // Arrange
            ...
            // Assert
            Assert.True(result.IsRight || result.IsLeft);
            result.IfRight(device => Assert.Equal(target, device.IpAddress));
        }

        #endregion

        #region ContinuousPingAsync Tests

        [Fact]
        public async Task ContinuousPingAsync_WithLoopback_ShouldReturnResults()
        {
            // Arrange
            var target = IPAddress.Loopback;
            var count = 3;
            ...
            // Act
            var result = await NetworkAnalyzerEngine.ContinuousPingAsync(target, count);
        ...
        [Fact]
        public async Task ContinuousPingAsync_WithDefaultParameters_ShouldWork()
        {
            // Arrange
            var target = IPAddress.Loopback;

            // Act
            var result = await NetworkAnalyzerEngine.ContinuousPingAsync(target);

            // Assert
            Assert.True(result.IsRight);
            result.IfRight(results =>
            {
                Assert.Equal(4, results.Count); // Default count is 4
                Assert.All(results, r => Assert.Equal(target, r.Target));
            });
        }

        [Fact]
        public async Task ContinuousPingAsync_WithCustomInterval_ShouldRespectInterval()
        {
            // Arrange
            var target = IPAddress.Loopback;
            var count = 2;
            ...
            {
                Assert.Equal(count, results.Count);
                // Should take at least (count-1) * interval time
                var expectedMinDuration = interval * (count - 1);
                Assert.True(endTime - startTime >= expectedMinDuration);
            });
        }

        [Fact]
        public async Task ContinuousPingAsync_WithCustomPingConfig_ShouldUseConfig()
        {
            // Arrange
            var target = IPAddress.Loopback;
            var customConfig = new PingConfiguration(500, 16, 32, false);
            var count = 2;

            // Act
            var result = await NetworkAnalyzerEngine.ContinuousPingAsync(target, count, null, customConfig);
        ...
        [InlineData(5)]
        [InlineData(10)]
        public async Task ContinuousPingAsync_WithDifferentCounts_ShouldReturnCorrectNumber(int count)
        {
            // Arrange
            var target = IPAddress.Loopback;
        ...
        [Fact]
        public async Task ContinuousPingAsync_WithUnreachableTarget_ShouldReturnFailedResults()
        {
            // Arrange
            var target = IPAddress.Parse("192.0.2.1"); // RFC 5737 test address
            var count = 2;
            ...
            // Act
            var result = await NetworkAnalyzerEngine.ContinuousPingAsync(target, count, null, quickConfig);
            ...
            result.Match(
                ...
                Left: error =>
                {
                    ...
                    Assert.Equal(target, pingError.Target);
                }
            );
        }
